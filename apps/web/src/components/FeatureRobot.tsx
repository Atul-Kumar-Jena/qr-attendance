  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (mode === 'jump') {
        ...
        else if (mode === 'idle') { const h = homeC(); tx = clamp(h.x + (mx - h.x) * 0.2, 70, innerWidth - 70); ty = clamp(h.y + (my - h.y) * 0.12, innerHeight * 0.5, innerHeight - 40) + Math.sin(now * 0.002) * 5; ts = 0.42; }
