...
export function Features() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const total = (FEATS.length - 1) * 360;
      ...
      gsap.utils.toArray<HTMLElement>('.feat-card').forEach((card, i) => {
        gsap.from(card, {
          opacity: 0.35, scale: 0.93, y: 24,
          duration: 0.8, ease: 'power2.out', delay: i * 0.04,
        });
      });
    }, root);
    return () => ctx.revert();
      ...
      <div className="hidden md:block overflow-hidden">
        <div ref={track} className="flex gap-6 pl-[10vw] will-change-transform">
          {FEATS.map((f, i) => (
            <article
              key={f.t}
              className="feat-card group relative shrink-0 w-[340px] aspect-[3/4] rounded-3xl glass p-8 flex flex-col justify-between hover:bg-cream-100/70 dark:hover:bg-white/4 transition-colors cursor-default"
            >
              {/* Animated illustration */}
              <div className="flex items-start justify-between">
                <div className="p-3 rounded-2xl bg-accent/6 border border-accent/12 group-hover:bg-accent/10 group-hover:border-accent/22 transition-colors">
                  <f.Illu />
                </div>
              ...
              <div>
                <h3 className="font-display text-[1.85rem] leading-tight">{f.t}</h3>
                <p className="mt-2 text-[13px] text-ink-mute">{f.d}</p>
              </div>
              <div className="absolute inset-x-8 bottom-7 h-px bg-ink/10 dark:bg-white/10" />
            </article>
          ))}
        </div>
      </div>
